...
const VOICE = {
  ...
  /* ---------- Why this workout ---------- */
  // Daily strength + evening stretch, straight from the coaching opinions:
  // strength builds gradually and the run plan always leads; stretching is
  // after runs and at night - hold, never bounce, hip flexors and hamstrings first.
  const STRENGTH = {
    cadence: 'After an easy run, 2-3 times a week. Build gradually - add a set only after two comfortable weeks.',
    moves: [
      { name: 'Squats', dose: '2 x 10' },
      { name: 'Walking lunges', dose: '2 x 8 each leg' },
      { name: 'Single-leg deadlifts', dose: '2 x 8 each leg' },
      { name: 'Calf raises', dose: '2 x 15' },
      { name: 'Plank', dose: '2 x 30 sec' },
    ],
  };
  const STRETCH = {
    cadence: 'Every evening, or right after your run. Hold each one - never bounce, never through pain.',
    moves: [
      { name: 'Kneeling hip-flexor stretch', dose: '30 sec each side' },
      { name: 'Supine hamstring stretch', dose: '30 sec each side' },
      { name: 'Figure-4 glute stretch', dose: '30 sec each side' },
      { name: 'Standing calf stretch', dose: '30 sec each side' },
    ],
  };
