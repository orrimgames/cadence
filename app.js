...
function renderToday() {
  ...
  $('#view').innerHTML = `
    <div class="page">
      ...
      ${sessHtml}
      <div class="statrow">
        <div class="stat"><b>${Engine.fmtMi(done)}</b><span>of ${Engine.fmtMi(wk.targetMi)} mi this week</span></div>
        <div class="stat"><b>${streak}</b><span>run streak</span></div>
...
function computeStreak() {
  let streak = 0;
  const days = new Set(S.runs.map(r => r.date));
  let d = Engine.todayISO();
  if (!days.has(d)) d = Engine.addDays(d, -1); // today not run yet doesn't break it
  while (days.has(d)) { streak++; d = Engine.addDays(d, -1); }
  return streak;
}
...
function renderRunSummary(run) {
  ...
  $('#view').innerHTML = `
    <div class="page">
      ...
      <canvas id="routecanvas" class="routecanvas" width="640" height="360"></canvas>
...
function renderStats() {
  ...
  const bars = weeks.map(k => {
    ...
    return `<div class="barcol"><div class="barwrap"><div class="bar ${cur ? 'cur' : ''}" style="height:${Math.max(h, v > 0 ? 6 : 2)}%"></div></div><span>${shortDate(k)}</span><b>${v ? Engine.fmtMi(v) : ''}</b></div>`;
  }).join('');
