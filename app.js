...
function startOnboarding() {
  ob = { step: 0, goal: null, raceDate: null, fitnessMode: 'race', raceDist: '5k', raceH: 0, raceM: 25, raceS: 0, easyMin: 10, easySec: 0, weeklyMiles: 0, daysPerWeek: 4, longDay: 6, units: 'mi', experience: null, age: 0, injuryNotes: '' };
  renderOnboarding();
}
...
function renderOnboarding() {
  ...
  const dots = `<div class="dots">${[0,1,2,3,4,5].map(i => `<i class="${i <= ob.step ? 'on' : ''}"></i>`).join('')}</div>`;
  ...
  if (ob.step === 3) {
    v.innerHTML = `
      <div class="ob">
        ...
        <div class="seg small">
          <button class="${ob.units === 'mi' ? 'on' : ''}" onclick="obSet('units','mi')">Miles</button>
          <button class="${ob.units === 'km' ? 'on' : ''}" onclick="obSet('units','km')">Kilometers</button>
        </div>
        <div class="seg">
        ...
        ${ob.fitnessMode === 'race' ? `
          ...
          <div class="steppers">
            ...
            <div class="unit">/ ${ob.units === 'km' ? 'km' : 'mile'}</div>
          </div>`}
  ...
  if (ob.step === 5) {
    const exps = [
      ['new', 'New to running', 'First time training with structure'],
      ['returning', 'Getting back into it', 'Ran before, then life happened'],
      ['seasoned', 'Seasoned', 'Consistent for a year or more'],
    ];
    v.innerHTML = `
      <div class="ob">
        ${dots}
        <h2>About you</h2>
        <p class="hint">Cadence tunes how much training you get - ambitious but honest.</p>
        <div class="cards">
          ${exps.map(([k, t, s]) => `
            <button class="opt ${ob.experience === k ? 'sel' : ''}" onclick="obSet('experience','${k}')">
              <b>${t}</b>
              <span>${s}</span>
            </button>`).join('')}
        </div>
        <p class="hint" style="margin-top:16px">Age <span class="dim">(optional)</span></p>
        <div class="steppers">
          <div class="stepper">
            <button onclick="obAgeBump(-1)">−</button>
            <div><b id="st_age">${ob.age ? ob.age : '—'}</b><span>years</span></div>
            <button onclick="obAgeBump(1)">+</button>
          </div>
        </div>
        <p class="hint" style="margin-top:16px">Anything nagging? <span class="dim">(optional)</span></p>
        <input type="text" class="dateinput" placeholder="e.g. left knee, tight calves" value="${esc(ob.injuryNotes || '')}" oninput="ob.injuryNotes=this.value">
        ${ob.injuryNotes && ob.injuryNotes.trim() ? `<p class="hint" style="color:var(--volt)">Noted - volume trimmed 10% and intensity kept conservative.</p>` : ''}
        <div class="navrow">
          <button class="ghost" onclick="obStep(4)">Back</button>
          <button class="cta" ${ob.experience ? '' : 'disabled'} onclick="buildPlan()">Build my plan</button>
        </div>
      </div>`;
    return;
  }
...
function obAgeBump(d) {
  ob.age = (ob.age || 0) + d;
  if (ob.age < 13) ob.age = 0;
  if (ob.age > 90) ob.age = 90;
  const el = $('#st_age');
  if (el) el.textContent = ob.age ? ob.age : '—';
}
